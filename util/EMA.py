...
class EMACallback(Callback):
    """Exponential Moving Averaging (EMA) callback for PyTorch Lightning.

    Maintains moving averages of trained parameters. Uses EMA weights
    during evaluation and saves them alongside original weights.
    ...
    Args:
        ...
        evaluate_ema_weights_instead: Use EMA weights for validation.
        save_ema_weights_in_callback_state: Save EMA weights in callback state.
    """
    ...
    def __init__(
        ...
        if not apex_available:
            rank_zero_warn(
                "EMA has better performance when Apex is installed: "
    ...
    def apply_ema(self, pl_module: "pl.LightningModule") -> None:
        for orig_weight, ema_weight in zip(list(pl_module.state_dict().values()), self._ema_model_weights):
            if orig_weight.data.shape == ema_weight.data.shape:
    ...
    def on_load_checkpoint(
        ...
        if trainer.ckpt_path and checkpoint_callback is not None \
            ...
            if os.path.exists(ema_path):
                ...
                warnings.warn(
                    "Unable to find associated EMA weights when re-loading, "
